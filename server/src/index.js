...
// Suppress canvas warnings and ES module warnings across all console methods
...
// Suppress specific Node.js warnings at the process level
process.removeAllListeners('warning');
process.on('warning', (warning) => {
  // Filter out ES module warnings and other unwanted warnings
  if (warning.message && (
    warning.message.includes('To load an ES module') ||
    warning.message.includes('set "type": "module"') ||
    warning.message.includes('use the .mjs extension') ||
    warning.message.includes('@napi-rs/canvas') ||
    warning.message.includes('canvas package')
  )) {
    return; // Ignore these warnings
  }
  
  // Log other warnings normally
  originalConsoleWarn(`Warning: ${warning.message}`);
  if (warning.stack) {
    originalConsoleWarn(warning.stack);
  }
});
